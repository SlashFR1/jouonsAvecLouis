/* --- 1. RÉCUPÉRATION DU LIEN (Doit être la toute première ligne) --- */
...
style.innerHTML = `
  ...
  /* Overlay : complètement retiré du rendu quand fermé */
  #bg-overlay {
    position: fixed;
    inset: 0;
    ...
    display: none; /* Important : caché au lieu de juste transparent */
  }
  #bg-overlay.show {
    display: block;
    opacity: 1;
...
menu.innerHTML = `
  ...
  <div id="bg-sidebar">
    ...
    <button onclick="closeMenu()" style="margin-top:auto; background:#ff6b6b; color:white;">Fermer</button>
  </div>
...
/* --- 4. GESTES MOBILE (Swipe pour fermer) --- */
let startX = 0;
...
sidebar.addEventListener("touchend", e => {
  ...
  if (diff < -60) closeMenu(); // Swipe vers la gauche → fermer
});
...
window.openRules = function() {
  ...
  document.getElementById("bg-modal-content").innerHTML =
    src ? src.innerHTML : `<h2>Oups</h2><p>Pas de règles définies sur cette page.</p>`;
...
/* --- 6. FERMETURE AUTOMATIQUE AU CHANGEMENT DE PAGE --- */
